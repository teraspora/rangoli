function  init_overlay() {
    // const suffices = ['B', 'C', 'D', 'E', 'J'];
    next_index = count => Math.floor(Math.random() * count);
    const imgs = document.getElementsByClassName(`rang_img`);
    const num_imgs = imgs.length;
    let current = next_index(num_imgs);
    let last;
    imgs[current].style.opacity = 1;
    setInterval(_ => {
        last = current;
        current = next_index(num_imgs);
    ...
    // setInterval(_ => {
    //     [...imgs].forEach(img => {
    //         img.style.transform = `rotate(${phi++}deg)`;
    //     });
    // }, 200);
}
...
document.onreadystatechange = () => {
    if (document.readyState === `complete`) {
        ...
        // Get reference to canvas etc., set dimensions
